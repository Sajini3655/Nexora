...
function isStoryPointDone(storyPoint) {
  const status = String(storyPoint?.status || "").trim().toUpperCase();
  return status === "DONE" || status === "COMPLETED" || storyPoint?.completed === true;
}
...
export default function DevTaskView() {
  ...
  const storyPointRows = Array.isArray(storyPoints) ? storyPoints : [];
  ...
  const budgetPoints = useMemo(() => {
    return numberOrZero(
      ...
      task?.storyPoints ??
      task?.totalEstimate ??
      progressData?.estimatedPoints ??
      progressData?.budgetPoints ??
      0
    );
  }, [task, progressData]);

  const storyPointTotals = useMemo(() => {
    const allocatedPointValue = storyPointRows.reduce(
      (sum, row) => sum + numberOrZero(row?.pointValue),
      0
    );

    const completedPointValue = storyPointRows.reduce(
      (sum, row) => sum + (isStoryPointDone(row) ? numberOrZero(row?.pointValue) : 0),
      0
    );

    const totalStoryPoints = storyPointRows.length;
    const completedStoryPoints = storyPointRows.filter(isStoryPointDone).length;

    const progressPercentage =
      allocatedPointValue > 0
        ? Math.round((completedPointValue * 100) / allocatedPointValue)
        : 0;
    ...
    return {
      ...
      progressPercentage,
      remainingPointValue: Math.max(0, budgetPoints - allocatedPointValue),
    };
  }, [storyPointRows, budgetPoints]);
  ...
  const handleToggleStoryPoint = async (storyPoint) => {
    ...
    try {
      if (isStoryPointDone(storyPoint)) {
  ...
  const handleCreateStoryPoint = async () => {
    const pointValue = numberOrZero(newPoints);

    if (!task?.id) return;
    if (!newTitle.trim()) {
      setStoryPointError("Enter a subtask title.");
      return;
    }
    if (pointValue <= 0) {
      setStoryPointError("Point value must be at least 1.");
      return;
    }
    if (budgetPoints <= 0) {
      setStoryPointError("Manager has not assigned a story point budget for this task.");
      return;
    }
    if (pointValue > storyPointTotals.remainingPointValue) {
      setStoryPointError(`Only ${storyPointTotals.remainingPointValue} point(s) remaining.`);
      return;
    }
        ...
        <Grid item xs={12} md={7}>
          <Card sx={{ p: 3, height: "100%" }}>
            ...
            <Box sx={{ mt: 3, display: "grid", gridTemplateColumns: { xs: "1fr", sm: "repeat(3, 1fr)" }, gap: 2 }}>
              <Metric label="Progress" value={`${storyPointTotals.progressPercentage}%`} />
                ...
                <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems="center">
                  ...
                  <Button
                    ...
                    disabled={
                      ...
                      budgetPoints <= 0 ||
                      storyPointTotals.remainingPointValue <= 0 ||
                      numberOrZero(newPoints) > storyPointTotals.remainingPointValue
                    }
