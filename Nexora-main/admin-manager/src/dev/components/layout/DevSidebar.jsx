...
export default function DevSidebar({ collapsed }) {
  ...
  const menuItems = [
    { name: "Dashboard", icon: <FiHome />, path: "/dev", moduleKey: "DASHBOARD" },
    { name: "Profile", icon: <FiUser />, path: "/dev/profile" },
    { name: "Chat", icon: <FiMessageCircle />, path: "/dev/chat", moduleKey: "CHAT" },
  ].filter(item => !item.moduleKey || hasPermission(item.moduleKey));
