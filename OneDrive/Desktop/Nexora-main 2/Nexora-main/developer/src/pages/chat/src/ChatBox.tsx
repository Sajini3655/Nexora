...
const ChatBox: React.FC<ChatBoxProps> = ({ projectId, onSummary }) => {
  ...
  const handleEndChat = async () => {
    try {
      const result = await endChat(chat, projectId); // taskId = projectId here
      setSummaryData(result);

      if (result.blockers?.length > 0 && result.ticket_prompt_needed) {
        setShowTicketBubble(true);
      } else {
        setChat((prev) => [...prev, { user: "assistant", message: `📄 Summary: ${result.summary}` }]);
        onSummary(result);
      }
    } catch {
